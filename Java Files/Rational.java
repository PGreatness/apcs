...
public class Rational implements Comparable
{
   ...
   *********************/
    ...
    {
	//might have extra stuff but works
	Rational obj = (Rational) other;
	if (!(obj instanceof Rational)) {
	    ...
	    reduce();
	    int otherHold = this._numerator * obj._denominator;
	    int thisHold = this._denominator * obj._numerator;
	    if (thisHold == otherHold) {
		return true;
	    }else{
		return false;
	    }
  ...
  {
      //probably doesn't have extra stuff but works
      
    // If other is not a Rational, throw an exception
    // This will exit the function, generating a runtime error
      Rational obj = (Rational) other;
      if (!(obj instanceof Rational)) {
	  throw new ClassCastException("Not of class Rational");
      }else{
	  if (this.equals(obj)) {
	      return 0;
	  }else{
	      int side1 = this._numerator * obj._denominator;
	      int side2 = this._denominator * obj._numerator;
	      if (side1 > side2) {
		  return -1;
	      }else{
		  return 1;
